...
class ModelComboService {
  static validateComboInput(name, modelsList, currentComboId = null) {
    ...
    allModels.forEach(m => {
      ...
      // Also map provider-prefixed variants (e.g. "groq_groq_model" → "groq_model")
      if (m.providerId && m.modelId) {
        const prefixed = `${m.providerId}_${m.modelId}`;
        modelMap.set(prefixed, m.id);
      }
    });
  ...
  static createCombo({ name, strategy, description, modelsList }) {
    ...
    const validStrategies = [
      'Fallback', 'Round Robin', 'Lowest Latency', 'Weighted',
      'Cost Optimized', 'Least Used', 'Power of 2 Choices', 'LKGP',
      'Auto', 'Fill First', 'Headroom', 'Reset Aware', 'Reset Window',
      'Strict Random', 'Context Optimized'
    ];
