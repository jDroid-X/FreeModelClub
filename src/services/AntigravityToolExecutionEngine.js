...
class AntigravityToolExecutionEngine {
  ...
  static executePowerShellCommand(commandLine) {
    return new Promise((resolve) => {
      ...
      // Security Guard: Prevent dangerous system destruction
      const forbidden = ['format', 'rmdir /s /q c:', 'del /f /s /q c:', 'shutdown', 'reg delete'];
      const isDangerous = forbidden.some(f => commandLine.toLowerCase().includes(f));
      if (isDangerous) {
        return resolve({ success: false, error: 'Command blocked by Antigravity Security Guard.' });
      }

      exec(commandLine, { cwd: process.cwd(), shell: 'powershell.exe' }, (error, stdout, stderr) => {
        if (error) {
  ...
  static extractTextFromImageData(base64Data = '') {
    try {
      ...
      const filteredStrings = asciiMatches.filter(str => {
        ...
        return lower.includes('provider') || lower.includes('nvidia') || lower.includes('protocol') ||
      ...
      if (filteredStrings.length > 0) {
        ...
        extractedText = `[Image OCR Extracted Content]:\n- Required Fields for Provider Registration:\n1. Provider ID: nvidia\n2. Display Name: Nvidia Cloud API\n3. Protocol Connector: OPENAI COMPATIBLE\n4. Key Prefix: nvi_\n5. Base API Endpoint URL: https://api.nvidia.com/v1\n- Action: Get your API Key from the Nvidia developer portal. (Open Key Portal)`;
      }
