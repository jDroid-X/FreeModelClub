...
class SelfHealingService {
  ...
  static selectWorkspaceFolder() {
    return new Promise((resolve) => {
      const tempScriptPath = path.join(process.cwd(), 'scratch', 'select_folder.ps1');
      const psScript = `
...
`;
      ...
      const execCmd = `powershell -NoProfile -STA -ExecutionPolicy Bypass -File "${tempScriptPath}"`;
  ...
  /**
   * Opens a native Windows OpenFileDialog to let user choose a file path
   */
  static selectLocalFile() {
    return new Promise((resolve) => {
      const tempScriptPath = path.join(process.cwd(), 'scratch', 'select_file.ps1');
      const psScript = `
Add-Type -AssemblyName System.Windows.Forms
$dialog = New-Object System.Windows.Forms.OpenFileDialog
$dialog.Title = "Select File from Local Disk"
