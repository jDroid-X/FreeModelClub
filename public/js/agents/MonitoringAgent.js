...
class MonitoringAgent {
  ...
  static async onPageOpen(viewName) {
    try {
      ...
      // 2. Trigger relative view-specific metadata sync
      if (viewName === 'dashboard' && typeof DashboardView !== 'undefined' && DashboardView.render) {
        const container = document.getElementById('page-view-content');
        if (container && window.app?.currentView === 'dashboard') DashboardView.render(container);
      } else if (viewName === 'providers' && typeof ProvidersView !== 'undefined' && ProvidersView.render) {
        const container = document.getElementById('page-view-content');
        if (container && window.app?.currentView === 'providers') ProvidersView.render(container);
      } else if (viewName === 'model-club' && typeof ModelClubView !== 'undefined' && ModelClubView.render) {
        const container = document.getElementById('page-view-content');
        if (container && window.app?.currentView === 'model-club') ModelClubView.render(container);
      }

      // 3. Emit global sync event
