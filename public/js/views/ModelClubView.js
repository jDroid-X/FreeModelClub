/**
 * ModelClubView.js
 * Purpose: Model Club view rendering 2-column layout with Combo Club Hierarchy Tree Navigation (< 140 lines).
...
class ModelClubView {
  ...
  static expandedBranches = { combos: true, skills: false, family: false, providers: false };
  ...
  static async render(container) {
    container.innerHTML = `
      <div class="glass-panel">
        <div class="panel-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
          <div class="panel-title"><i class="fa-solid fa-cubes-stacked"></i> Model Club Taxonomy & Models Combo</div>
    ...
    try {
      ...
      if (typeof TaxonomyHelper !== 'undefined') {
        const pyramid = TaxonomyHelper.buildPyramid(modelArray, this.allProviders, this.combos);
        this.allModels = pyramid.models;
    ...
    const content = `
      <form id="edit-model-config-form" style="display: flex; flex-direction: column; gap: 8px; font-size: 0.8rem;">
        <div style="background: rgba(6,182,212,0.08); border: 1px solid var(--accent-cyan); padding: 8px 10px; border-radius: 6px;">
          <strong style="color: var(--accent-cyan);"><i class="fa-solid fa-microchip"></i> Editing Model Config: ${m.modelName || m.name || m.id || 'Model'}</strong>
          <div style="font-size: 0.72rem; color: var(--text-muted);">Unique ID: <code>${m.id || ''}</code> | Provider: <strong>${m.providerName || m.providerId || 'System'}</strong></div>
        </div>

        <div class="grid-2" style="gap: 8px;">
          <div class="form-group" style="margin-bottom:0;">
            <label class="form-label" style="font-size: 0.75rem; font-weight: 700;">Model Display Name:</label>
            <input type="text" id="cfg-model-name" class="form-control" style="font-size: 0.78rem;" value="${m.modelName || m.name || ''}" required />
          </div>
          <div class="form-group" style="margin-bottom:0;">
