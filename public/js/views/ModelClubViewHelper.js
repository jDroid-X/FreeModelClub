...
class ModelClubViewHelper {
      ...
      <div class="glass-card tile-card" style="padding: 14px;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
          <h4 style="color: var(--text-main); font-size: 0.95rem; margin: 0; font-weight: 700;">
            ${typeof FormatHelper !== 'undefined' ? FormatHelper.sanitizeModelName(m.modelName || m.modelId) : (m.modelName || m.modelId)}
          </h4>
        ...
        <div style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 10px; display: flex; flex-direction: column; gap: 3px;">
          <div><i class="fa-solid fa-server" style="width: 16px;"></i> Provider: <strong style="color: var(--primary-light);">${m.providerName}</strong></div>
        ...
        <div style="max-height: 450px; overflow-y: auto;">
          <table class="table-custom" style="width: 100%; font-size: 0.8rem;">
            ...
            <tbody>
              ${activeModels.map(m => `
                <tr>
                  <td><strong style="color: var(--text-main);">${typeof FormatHelper !== 'undefined' ? FormatHelper.sanitizeModelName(m.modelName || m.modelId) : (m.modelName || m.modelId)}</strong></td>
                  <td><span class="badge badge-steel">${m.providerName}</span></td>
  ...
  static renderComboCardHtml(c) {
    const strategyBadge = c.strategy === 'Round Robin' ? 'badge-steel' : (c.strategy === 'Fallback' ? 'badge-amber' : 'badge-active');
