...
class ChatStreamView {
  static renderMessages(activeSession) {
    ...
    container.innerHTML = window.app.chatHistory.map((m, idx) => `
      <div class="chat-msg ${m.role}" style="display: flex; gap: 6px; ${m.role === 'user' ? 'justify-content: flex-end;' : 'justify-content: flex-start;'}">
        <div style="max-width: 85%; padding: 4px 8px; border-radius: 6px; background: ${m.role === 'user' ? 'rgba(56, 189, 248, 0.15)' : 'rgba(255,255,255,0.03)'}; border: 1px solid ${m.role === 'user' ? 'var(--accent-cyan)' : 'var(--border-color)'};">
