...
class ManualView {
  ...
  static async render(container) {
    ...
    ManualView.currentManualSteps = manualSteps;
    ManualView.currentFaqs = faqs;
      ...
      <div class="glass-panel">
        ...
        <div style="display: flex; gap: 16px; align-items: flex-start; margin-top: 12px;">
          ...
          <div class="glass-panel" style="width: 20%; min-width: 170px; flex-shrink: 0; padding: 10px; display: flex; flex-direction: column; gap: 6px; position: sticky; top: 10px;">
            ...
            `).join('')}
            <button class="btn btn-secondary btn-sm toc-link" style="justify-content: flex-start; font-size: 0.72rem; padding: 6px;" onclick="ManualView.scrollToSection('sec-checklist')">
              <i class="fa-solid fa-check-double" style="color: var(--accent-cyan);"></i> Setup Checklist
            </button>
    ...
    `;
    // BUG-2 fix: Mount checklist AFTER innerHTML is set (was never called before).
    // Delegate to DiagnosticChecklist.renderFull() — single source of truth.
    const checklistEl = document.getElementById('diagnostic-checklist');
    if (checklistEl && typeof DiagnosticChecklist !== 'undefined') {
      DiagnosticChecklist.renderFull(checklistEl);
    }
  }

  // BUG-7 fix: renderChecklist() now delegates to DiagnosticChecklist (no duplicate logic).
  static async renderChecklist() {
    const el = document.getElementById('diagnostic-checklist');
    if (el && typeof DiagnosticChecklist !== 'undefined') {
      await DiagnosticChecklist.renderFull(el);
    }
  }

  // BUG-10 fix: refreshChecklist() calls renderFull directly — no outerHTML swap.
  static async refreshChecklist() {
  ...
  static exportManualMarkdown() {
    let text = `# FreeModelsClub Localhost Smart Chatbot - Operational Manual\n\n`;
    
    if (ManualView.currentManualSteps && ManualView.currentManualSteps.length > 0) {
      ManualView.currentManualSteps.forEach(m => {
        text += `## ${m.step}. ${m.title}\n${m.description}\n\n`;
        if (m.hilAction) {
          text += `**Human-In-Loop Action:** ${m.hilAction}\n\n`;
        }
      });
    } else {
      text += `*No manual steps available.*\n\n`;
    }

    if (ManualView.currentFaqs && ManualView.currentFaqs.length > 0) {
      text += `## Frequently Asked Questions (FAQ)\n\n`;
      ManualView.currentFaqs.forEach((faq, index) => {
        // Strip HTML tags for markdown export
        const cleanA = faq.a.replace(/<[^>]+>/g, '');
        text += `### Q${index + 1}: ${faq.q}\n**A:** ${cleanA}\n\n`;
      });
    }
  ...
  static printManual() {
    ModalDialog.showOptionModal({
      title: 'Print & Export HIL User Manual',
      message: 'Select export target for the Human-In-Loop User Manual:',
      icon: 'fa-print',
      options: [
        {
          id: 'print-window',
          label: 'Printable HTML Window (Printer / PDF)',
          icon: 'fa-print',
          type: 'primary',
          action: () => {
            const printWin = window.open('', '_blank');
            if (!printWin) {
              ModalDialog.showNotification('Pop-up blocked. Please allow pop-ups to print manual.', 'warning');
              return;
            }
            const content = document.getElementById('manual-steps-container')?.innerHTML || '';
            printWin.document.write(`
              <!DOCTYPE html>
              <html>
                <head>
                  <title>FreeModelsClub Operational User Manual</title>
                  <style>
                    body { font-family: system-ui, -apple-system, sans-serif; padding: 24px; color: #0f172a; line-height: 1.6; }
                    h1, h2, h3 { color: #0284c7; }
                    .manual-step-card { border: 1px solid #cbd5e1; padding: 16px; margin-bottom: 16px; border-radius: 8px; }
                    .btn, button, input { display: none !important; }
                  </style>
                </head>
                <body>
                  <h1>FreeModelsClub Localhost Smart Chatbot - Human-In-Loop User Manual</h1>
        ...
        {
          id: 'md-file',
          label: 'Download Markdown Documentation (.md)',
          icon: 'fa-file-arrow-down',
          type: 'cyan',
          action: () => {
            ManualView.exportManualMarkdown();
          }
        }
      ]
    });
