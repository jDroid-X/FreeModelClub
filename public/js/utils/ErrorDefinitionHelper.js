 ...
 * Purpose: Frontend Browser Master Error Code Dictionary & Guidance Card Renderer.
 */
...
class ErrorDefinitionHelper {
  static getDictionary() {
    return {
      ...
      'ERR_PROVIDER_NOT_FOUND': {
        ...
        title: 'Provider Search Notice (Specs Not Found)',
        definition: 'Live AI search and registry query could not automatically deduce API specs for this query.',
        guidance: 'Verify provider spelling or domain name, or enter Base API URL and Protocol manually in Provider Registration.',
        actionBadge: 'Manual Entry',
        color: 'var(--accent-amber)'
      }
    };
  }
  ...
  static renderErrorCardHtml(errInfo) {
    const info = typeof errInfo === 'string' ? this.getByStatusCode(null, errInfo) : (errInfo && errInfo.code ? errInfo : this.getByStatusCode(errInfo ? errInfo.statusCode : null, errInfo ? (errInfo.error || errInfo.message) : ''));
...
window.ErrorDefinitionHelper = ErrorDefinitionHelper;
